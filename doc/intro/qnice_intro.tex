   ...
   \begin{frame}
    ...
    \begin{itemize}
     \item QNICE utilizes 18 basic instructions, all of which (apart
      from the {\tt CTRL} group, which takes one optional operand) are two 
    ...
    \begin{itemize}
     \item Most of QNICE's instructions feature a single instruction 
      ...
      \begin{center}
       ...
       \end{tabular}
      \end{center}
     \item The control instructions have the following general format:
      \begin{center}
       \begin{tabular}{|c||c|c||c|}
        \hline
         4 bit&4 bit&2 bit&6 bit\\
         {\tt opcode}&{\tt src rxx}&{\tt src mode}&\texttt{command}\\
        \hline
       \end{tabular}
      \end{center}
    ...
    \begin{center}
     \begin{tabular}{|c|ll|l|}
      ...
      \hline
       ...
       {\tt D}&reserved&\\
       {\tt E}&{\tt CTRL}&&\texttt{HALT}, \texttt{INT}, \texttt{RTI}\\
   ...
   \begin{frame}
    ...
    \begin{center}
     \begin{tabular}{l|l|l}
      ...
      \texttt{000001}&\texttt{INT}&Issue a software interrupt with the\\
       &&address supplied by the source operand\\
      \texttt{000010}&\texttt{RTI}&Return from interrupt\\
     \end{tabular}
    \end{center}
   \end{frame}
%
  ...
  \begin{frame}
   ...
   \begin{itemize}
    ...
    \item The processor has two sets of the registers \texttt{R14} (status
     ...
     be left by the control instruction \texttt{CTRL RTI}. This effectively
  ...
  \begin{frame}
   ...
   \begin{itemize}
    \item The CPU features two lines to deal with external interrupts:
     ...
     \end{description}
    \item A software interrupt is triggered by \texttt{CTRL INT <source op>}.
     The source operand contains the address of the ISR.
    \item To return from an interrupt the instruction \texttt{CTRL RTI} is
     used.
   \end{itemize}
  \end{frame}
%
