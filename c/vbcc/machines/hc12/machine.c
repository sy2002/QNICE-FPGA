...
char *regnames[]={"noreg","d","x","y","sp","u"};
...
static int acc=1,ix=2,iy=3,sp=4,iu=5;
...
/*  Gibt Objekt auf Bildschirm aus                      */
{
  ...
  if((p->flags&(VAR|REG))==VAR){
	...
	emit(f,"(%s%s)",idprefix,p->v->identifier);
      ...
      if(pcrel&&ISFUNC(p->v->vtyp->flags))
	emit(f,",pc");
...
static void gen_pop(FILE *f,long l)
{
  ...
  if(l==1&&CPU==6812){
    ...
    emit(f,"\tleas\t%u,%s\n",((unsigned int)l)&0xffffu,regnames[sp]);
  }
  pop(l);
}
...
/*  erzeugt Funktionskopf                       */
{
  ...
  if(offset){
    if(CPU==6812&&offset==1)
      ...
      emit(f,"\tleas\t%u,%s\n",(unsigned int)(-offset)&0xffffu,regnames[sp]);
    have_frame=1;
  }
}
...
static void peephole(struct IC *p)
{
  ...
  for(;p;p=p->next){
    ...
    /* POST_INC/DEC in q2 */
    if(ISCHWORD(t)&&!p->q2.am&&(p->q2.flags&(REG|DREFOBJ))==(REG|DREFOBJ)){
      r=p->q2.reg; t=q2typ(p);
      if(ISIDX(r)&&(!(p->q1.flags&REG)||p->q1.reg!=r)&&(!(p->z.flags&REG)||p->z.reg!=r)){
	for(p2=p->next;p2;p2=p2->next){
	  if((p2->code==ADD||p2->code==ADDI2P||(CPU==6812&&(c2==SUB||c2==SUBIFP)))&&(p2->q1.flags&(REG|DREFOBJ))==REG&&(p2->z.flags&(REG|DREFOBJ))==REG&&p2->q1.reg==r&&p2->z.reg==r&&(p2->q2.flags&(KONST|DREFOBJ))==KONST){
	    eval_const(&p2->q2.val,p2->typf2);
	    if(c2==SUB||c2==SUBIFP) vmax=zmsub(l2zm(0L),vmax);
	    if(zmleq(vmax,l2zm(8L))&&zmleq(l2zm(-8L),vmax)){
	      p2->code=NOP;
    ...
    /* POST_INC/DEC in z */
    if(ISCHWORD(t)&&!p->z.am&&(p->z.flags&(REG|DREFOBJ))==(REG|DREFOBJ)){
      r=p->z.reg; t=ztyp(p);
    ...
    /* R,#c */
    if((c==ADDI2P||c==SUBIFP)&&ISCHWORD(p->typf)&&((p->typf2&NQ)==NPOINTER||(p->typf2&NQ)==FPOINTER)&&isreg(z)&&((p->q2.flags&(KONST|DREFOBJ))==KONST||(!drel&&(p->q1.flags&VARADR)))){
...
{
  int f=t->flags&NQ;
  if(ISSCALAR(f)&&(ISLWORD(f)||ISHWORD(f)||f==CHAR))
    return acc;
  return 0;
}
...
void gen_code(FILE *f,struct IC *fp,struct Var *v,zmax offset)
{
  ...
  for(p=fp;p;pr(f,p),p=p->next){
	...
	store_reg(f,p->q1.reg,&p->z,(t&NQ)==CHAR?t:INT);
      }
      continue;
    }
    if(c==CALL){
	...
	if(c==PUSH){
	  emit(f,"\tleas\t%u,%s\n",((unsigned int)(-size))&0xffffu,regnames[sp]);
	  push(size);
	}
	...
	if(size<=6||(size<=16&&!optsize)){
	  if(CPU!=6812){
	    if(!scratchreg(acc,p)){
	      if(c==PUSH)
		emit(f,"\tstd\t%ld,%s\n",loff-6-stackoffset,regnames[sp]);
	      else{
		emit(f,SPUSHD);
		push(2);
	      }
	...
	if(isreg(q1)){
	  ...
	  if(regs[reg]&&!scratchreg(reg,p)){
	    st=1;
	    emit(f,"\tst%s\t%ld,%s\n",regnames[reg],(long)loff-2-stackoffset,regnames[sp]);
	  }
	...
	if(isreg(q1)){
	  if(ISIDX(reg)&&ISIDX(p->q1.reg)&&isconst(q2)){
	    ...
	    if(CPU==6812&&p->q1.reg==reg&&zmeqto(vmax,l2zm(1L))&&zumeqto(vumax,ul2zum(1UL))){
	      ...
	      emit(f,"\tlea%s\t%u,%s\n",regnames[reg],c==SUB?(unsigned int)(-zm2l(vmax))&0xffffu:(unsigned int)(zm2l(vmax))&0xffff,regnames[p->q1.reg]);
	    }
	    opdone=1;
	...
	if(!opdone){
	  if(reg==acc){
		...
		emit(f,"\tlea%s\t%u,%s\n",regnames[reg],(unsigned int)(l)&0xffffu,regnames[reg]);
	      }
	    }else{
	      if(c!=ADD&&c!=ADDI2P){
		if(!ISRACC(q2)){
